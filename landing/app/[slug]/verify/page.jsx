...
export default function SlugVerify() {
  ...
  async function handleVerify(e) {
    ...
    try {
      ...
      router.push(`/${slug}/wallet-choice`);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-white px-4">
      ...
      <p className="text-gray-500 mb-6 text-sm">Sent to {sessionStorage.getItem?.('enroll_phone')}</p>
      <form onSubmit={handleVerify} className="w-full max-w-xs">
        ...
        <div className="text-center text-sm text-gray-500">
          {countdown > 0 ? <span>Resend in {countdown}s</span> : (
            <button type="button"
              onClick={async () => { await axios.post('/api/auth/send-otp', { phone: sessionStorage.getItem('enroll_phone') }); setCountdown(60); }}
              className="text-black underline font-medium">Resend OTP</button>
          )}
        </div>
      </form>
    </div>
